...
def restore_company(ticker: str) -> dict:
    """Switch the dashboard back to an already-run company. Free — no API calls.

    Rebuilds financials.csv/metadata.json from tableau_export_<TICKER>.csv and
    the dashboard commentary from docs/analysis_report_<TICKER>.md.
    """
    _banner(f"Restoring {ticker} from saved artifacts (no API calls, no cost)")
    meta = data_access.restore_from_export(ticker)
    print(f"  {meta['row_count']} rows, periods {meta['periods_covered'][0]} .. "
          f"{meta['periods_covered'][1]}, source={meta['source']}")

    report_path = PROJECT_ROOT / "docs" / f"analysis_report_{ticker}.md"
    if report_path.exists():
        report_md = report_path.read_text()
        cut = report_md.find("\n\n---\n\n> Generated by")  # strip the doc footer
        if cut != -1:
            report_md = report_md[:cut]
        data_access.write_analysis({
            "ticker": meta["ticker"], "source": meta["source"],
            "generated_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
            "restored": True,
            "categories": runners.split_report(report_md), "report_md": report_md,
        })
        print(f"  Dashboard commentary restored from {report_path.name}")
    else:
        print(f"  No saved report at {report_path.name} — commentary will show "
              "as unavailable until a full pipeline run")
    print("  Reminder: restored data is as of that company's last pipeline run — "
          "re-run --source edgar for fresh filings")
    return {"restored": ticker, "rows": meta["row_count"], "cost_usd": 0.0}
...
def main() -> int:
    ...
    parser.add_argument("--restore", metavar="TICKER",
                        help="Switch back to an already-run company from its saved "
                             "artifacts — free, offline, no API key needed")
    args = parser.parse_args()
    if args.restore:
        summary = restore_company(args.restore.upper())
        print(f"\nDone: {summary}")
        return 0
    try:
